...
```
So that is really bad results. I do make the poisson model anyways. And after also looking at residual deviance compared to the degrees of freedom it confirms over dispearsion and then I make a negative binomial model. \
\
Check for colinearity in predictors
```{r}
cor(bees[, c("effort", "altitude", "MAT")])
car::vif(m)
...
Check residuals
```{r}
plot(m$fitted.values, residuals(m, type="deviance"))
abline(h=0)
