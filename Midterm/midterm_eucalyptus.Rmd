...
# Added a random effect for landscape
m11 <- glm.nb(Total_seedings ~ 
            annual_precipitation + 
            Litter_cover +
            U_ppm + 
            ExoticPerennialGrass_cover + 
...
```{r}
 mlist = list(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11)
 AICTab = AIC(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11)
...
```
The model with 9 predictors (annual_precipitation, 
            Litter_cover,
            Landscape.position,
            U_ppm,
            ExoticPerennialGrass_cover,
...
# Plot results
```{r}
predictors <- c("annual_precipitation", 
            "Litter_cover",
            "Landscape.position",
            "U_ppm",
            "ExoticPerennialGrass_cover",
...
pred <-  ggpredict(m2, terms = predictors[i])
...
p <- ggplot(pred, aes(x = x, y = predicted)) +
  ...
  # add raw data points 
  geom_point(data = eucalyp, aes(x = .data[[predictors[i]]], y = Total_seedings),
             inherit.aes = FALSE,
             col = "darkred") +
  labs(x = predictors[i],
